...
function trainSeating (penumpang) {
 ...
 for (var i = 0; i < penumpang.length; i++) {
   if (penumpang[i].gerbong in hasil === false) {
     hasil[penumpang[i].gerbong] = []
   }
   if (hasil[penumpang[i].gerbong] !== 'VVIP' && hasil[penumpang[i].gerbong] !== 'VIP' && hasil[penumpang[i].gerbong] !== 'Regular') {
     hasil[penumpang[i].gerbong].push({ nama: penumpang[i].nama, seat: penumpang[i].seat})
   }
 }
